...
if(mysql_affected_rows()==0){//name does not exist in unit database
	...
	if(mysql_affected_rows()==0){// If no then add into main list of users
		mysql_query("INSERT INTO account(username, nickname, password, first_name, last_name, status, email) VALUES('$stud_name','anonymous','$pswd','$stud_fname','$stud_lname','$status','$stud_email')")  or die("Account not created! Check that your username has less than 20 characters.");
